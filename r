...
Building regression models for insurance claims poses several challenges. The process can be particularly difficult for individual insurance policies where a large proportion of claims are zeros and for those policies with a claim, the losses typically exhibit skewness. The degree of skewness in the positive loss distribution varies widely for different types of insurance {\color{blue}products}. For example, within a class of automobile insurance policies, the losses arising from damages to {\color{blue}the} property may be medium-tailed but those arising from liability-related claims may be long-tailed. For short-term insurance contracts, the number of claims is referred to as the claim frequency, while the amount of claims, conditional on the occurrence of a claim, is the claim severity. The addition of covariates, or explanatory variables, in a regression context within each component allows to capture the possible heterogeneity of the individual policies. These two components are used as predictive models for pure premiums{\color{blue}, expected loss costs,} and for aggregate or total claims in an insurance portfolio.

There is a long history of studying insurance claims based on the two-part framework. Several well-known families of distributions for claim frequencies and claim severities are well documented in \cite{klugman2012}. As more observed data became increasingly available, additional complexities to traditional regression linear models have been introduced in the literature and in practice; for example, {\color{blue}the use of zero-inflated} or hurdle models has been discussed to accommodate the shortcomings of traditional models for excess zeros. Interestingly, the Tweedie GLM for insurance claims have been particularly popular because it is adaptable to a mixture of zeros and non-negative insurance claims.  \cite{smyth2002fitting} described the Tweedie compound Poisson regression models and calibrated such models using a Swedish third party insurance claims dataset; the models were fitted to examine dispersion within the GLM framework. The Tweedie distribution can be regarded as a reparameterization of the compound Poisson-gamma distribution, so that model calibration can be done within the GLM framework. \citet{xacur2015generalised} compared these two approaches and concluded that there is no clear superior method; the paper also discussed the advantages and disadvantages of the two methods. {\color{blue}The} Tweedie GLM presents a larger pure premium and implies both a larger claim frequency and claim severity due to the constant scale (dispersion) parameter; in other words, the mean increases with {\color{blue}the} variance. The constant scale parameter also forces an artificial relationship between the claim frequency and the claim severity. {\color{blue}The} Tweedie GLM does not have an optimal coefficient, {\color{blue}especially under the inflexible distribution assumption,} and it leads to a loss of information because it ignores the number of claims. On the other hand, {\color{blue}the} Tweedie GLM has fewer parameters to estimate and is thus considered more parsimonious than a two-part framework. When insurance claims data contain small losses due to low frequencies, the two-part framework {\color{blue} \st{highly}} likely overlooks the internal connection between the low frequency and the subsequent small loss amount. For example, the frequency model often indicates zero number of claims for small claim policies, which leads to a zero loss prediction. Other works related to {\color{blue}the} Tweedie GLM can be found in \cite{frees2014predictive} and \cite{jorgensen1987exponential}. 

Within the GLM framework, the claim frequency component may not accurately accommodate imbalances caused by the large proportion of zero claims. Simultaneously, a significant limitation is that the regression structure of the logarithmic mean is restricted to a linear form, which may be too inflexible for real applications. With {\color{blue}the} rapid expansion of available data for improved decision making, there is a growing appetite in the insurance industry to expand its toolkit for data analysis and modeling. However, the industry is unarguably highly regulated so there is pressure for actuaries and data scientists to provide adequate transparency in modeling techniques and results. To find a balance between modeling flexibility and interpretation, in this paper, we propose a {\color{blue} parametric model empowered by tree-based methods} with a hybrid structure. 

Since \citet{breiman1984} introduced the Classification and Regression Tree (CART) algorithm, tree-based models have gained momentum as a powerful machine learning approach for decision making in several disciplines. Especially single-tree models, like the CART algorithm, can be interpreted straightforwardly by visualizing the tree structure. The CART algorithm involves separating the explanatory variable space into several mutually exclusive regions that, as a result, creates a nested hierarchy of branches resembling a tree structure. Each separation or branch is referred to as a node. Each of the bottom nodes of the decision tree, called terminal nodes, has a unique path for observable data to enter the region. Once the decision tree is constructed, it is possible to predict and locate the terminal node for scoring {\color{blue}the} dataset using a new set of observed explanatory variables.

Prior to further exploring the hybrid structure, it is worth mentioning that a similar structure called Model trees, with an M5 algorithm, was first described in \cite{quinlan1992learning}. In the M5 algorithm, it constructs tree-based piecewise linear models. While regression trees assign a constant value to the terminal node as the fitted value, Model trees use a linear regression model at each terminal node to predict the fitted value for observations that reach that terminal node. Regression trees are a special case of Model trees. Both regression trees and Model trees employ recursive binary splitting to build a fully grown tree. Thereafter, both algorithms use a cost-complexity pruning procedure to trim the fully grown tree back from each terminal node. The primary difference between regression trees and Model trees algorithms is that for the latter step, each terminal node is replaced by a regression model instead of a constant value. The explanatory variables that serve to build that regression models are generally those that participate in the splits within the subtree that will be pruned. In other words, explanatory variables in each node are located beneath the current terminal node in the fully grown tree.

It has been demonstrated that Model trees have advantages over regression trees in both model simplicity and prediction accuracy. Model trees produce decision trees considered not only relatively simple to understand, but are also efficient and robust. Additionally, such tree-based methods can exploit local linearity within the dataset. When prediction performance is compared, it is worth mentioning the difference in the range of predictions produced between traditional regression trees and Model trees. Accordingly, regression trees are only able to give a prediction within the range of observed values in the training dataset. However, Model trees are able to extrapolate {\color{blue}the} prediction range because of the use of the regression models at the terminal node. Especially, when the regression function is a smooth function of explanatory variables, the model tree may solve the problem that the regression tree may not fit the data well. For further discussion of Model trees and {\color{blue}the} M5 algorithm, see \cite{quinlan1992}.

Inspired by the structure and advantages of Model trees when compared to traditional regression trees, we develop a similar algorithmic structure that can uniquely capture the two-part nature of insurance claims. While it is true that Model trees and regression trees are used for the general regression task, insurance claim datasets present an additional layer of information, the frequency component, which is a classification task. In this paper, we propose the hybrid tree-based methods as a two-step algorithm: the first step builds a classification tree to identify the membership of claim occurrence, and the second step uses a regularized regression technique to determine the claim severity at each of the terminal nodes. In essence, such hybrid tree-based methods integrate the peculiarities of both the classification tree and the linear regression in the modeling process. These sets of models are suitably described as {\color{blue}a} hybrid structure, which has the appearance of a segregated two-step structure, but in actuality, it is an integrated approach so that it is more reasonably comparable to the Tweedie GLM than {\color{blue}the two-part framework}. Furthermore, in the model evaluation, it becomes unreasonable to compare the frequency and severity components in segregation.

The rest of this paper has been organized as follows. In Section \ref{sec:HT}, we provide technical details of the algorithm arising from hybrid tree-based methods applicable to insurance claims, separating the modeling of claim frequency and claim severity. In Section \ref{sec:sim}, to investigate the predictive accuracy of our hybrid tree-based methods, we create a synthetic dataset based on a simulation produced from a true Tweedie distribution. Within the simulation, we introduced some deviations to mimic real-life situations for a more reasonable comparison. In Section \ref{sec:APP}, using empirical data drawn from a portfolio of general insurance policies, we illustrate the calibration results and compare the prediction accuracy based on a hold-out sample for validation. Section \ref{sec:visual} provides visualization of the results from the hybrid structure, which accommodates better interpretation. We conclude in Section \ref{sec:conclude}. 

\section{Description of the hybrid tree-based methods} \label{sec:HT}

Hybrid tree-based methods utilize the information arising from both the frequency and the severity components. As already alluded {\color{blue}to} in Section \ref{sec:intro}, it is a two-stage procedure. In the first stage, we construct a classification tree-based model for frequency. In the subsequent stage, we employ a penalized regression model at each terminal node within the tree-based model, based on the severity information. Such methods can be drawn from the hybrid structure, which can be viewed as an ecosystem that is able to accommodate a variety of characteristics of datasets.
...
\end{equation}
where $H$ represents the response from the hybrid tree-based method, given the set of explanatory variables $\textbf{X}$, the tree structure $T(\Theta)$, and regression model coefficients $\mathbf{\boldsymbol{\beta}}_{s_{m}}$. From Equation (\ref{eq:1a}), we see that a hybrid structure may also be viewed as piecewise regression models. {\color{blue}The tree-based algorithms, $T(\Theta)$, divide the dataset into $M$ subsamples (or risk classes represented by $\mathbf{1}_{R_{m}}(\textbf{X})$) and then assign expected average number of claim $\beta_{f_{m}}$, frequency information. Based on this division, regression models with coefficients $\mathbf{\boldsymbol{\beta}}_{s_{m}}$ are applied to these subsamples. Each component is further described in the subsequent subsections.}

We can determine the type of classification trees that can be adapted in the frequency component according to the information drawn from the dataset. If it records only whether claims were reported or not, we construct a classification tree for a binary response variable. If it records additional information on the number of claims, we construct trees for a count response variable. For the binary classification, we can employ some of the most popular algorithms, to list a few: (a) the CART algorithm, (b) efficient algorithm C4.5 \citep{quinlan1992}, with a current updated version C5.0, (c) unbiased approaches like Generalized, Unbiased, Interaction Detection, and Estimation (GUIDE) \citep{loh2009improving}, or (d) Conditional Inference Trees (CTREE) \citep{hothorn2006unbiased}. For the count response variable, we can apply, to list a few: (a) piecewise linear Poisson using the CART algorithm, (b) SUPPORT \citep{chaudhuri1995generalized}, (c) Poisson regression using GUIDE \citep{loh2006regression}, or (d) MOdel-Based recursive partitioning (MOB) \citep{zeileis2008model}.

After completion of the classification tree structure, we employ a linear {\color{blue} regression} model to each of the terminal nodes. The simplest model includes GLM \citep{nelder1972glm} with different distribution models from this family together with regularized regression such as the elastic net regularization \citep{zou2005reg} amongst others.

Hybrid tree-based methods build an ecosystem that utilizes modern techniques from both traditional statistics and machine learning. In the subsequent subsections, for illustration purposes, we only show a simple hybrid tree-based method without exploring all possible combinations of the different algorithms suitable at each stage of the procedure. To illustrate, we select the well-known CART algorithm for binary classification for {\color{blue}the} claim frequency component and least squares regression with the elastic net regularization for {\color{blue}the} claim severity component. The elastic net regularization can perform variable selection and improve prediction accuracy when compared to traditional linear regressions without regularization. In order to be self-contained, we illustrate this simple hybrid tree-based method with enough details. Additional details for the severity models are in Appendix A. See also \cite{james2013}.

\subsection{Claim frequency component} \label{frequency}

We denote the response variable as \(\textbf{Y}\), the sample space as \(\mathcal{Y}\), and \(n\) as the number of observations. The \(i\)th {\color{blue} \st{sample} observation} with \(p\)-dimensional explanatory variables is denoted as \(\textbf{x}_i = (x_{i1},x_{i2}\ldots,x_{ip})\) \(i=1 \dots n\), which is sampled from the space \(\mathcal{X} = \mathcal{X}_{1} \times \ldots \times \mathcal{X}_{p}\). {\color{blue} \st{For example,} W}e can separate each claim \(y_i\) into \(y_{i_{f}}\), the claim occurrence or the number of claims, and \(y_{i_{s}}\), the claim severity.
...
\end{itemize}
where \(p\) is {\color{blue} the proportion of observations in the }one class in the node. The default CART algorithm uses {\color{blue}the} Gini index as a loss function. For multi-class loss functions, see \cite{hastie2009}.
...
\end{equation}
where \(w_.\) is the weight for the subregion determined by the number of observations split into the subregion divided by the total number of observations before the split, and \(p_.\) is the proportion of one class in the subregion. Subsequently, the algorithm looks for the next explanatory variable with the best division into two subregions and this process is applied recursively until meeting some predefined threshold or reaching a minimum size of observations at the terminal node.
...
The mathematical foundation for the elastic net regularization can be found in \citet{de2009elastic}. It should also be noted that all the regularization schemes mentioned previously have simple Bayesian interpretations. See \citet{hastie2009} and \citet{james2013}. A review paper for statistical learning applications in general insurance can be found in \citet{parodi2012computational}.

Based on hyperparameters $\lambda$ and $\alpha$, the linear structure varies among ordinary linear regression (when $\lambda=0$), ridge regression, LASSO, and the elastic net regularization. In addition, the hybrid tree-based methods perform automatic variable selections for claim severity, which is distinctively different from the Model trees mentioned in Section $\ref{sec:intro}$. Model trees consider only explanatory variables that are not used in the splitting process involved in the tree structure as candidates when it builds models to the terminal nodes. The reason for the elastic net is to amplify the information remaining after the classification step. This is partly because some explanatory variables may be significant for the claim frequency component, but not for the severity component. For this reason, the practice of choosing the correct set of explanatory variables is important and thereby, we employ the elastic net. In the extreme case where for example gender is an explanatory variable, only observations of one particular gender may remain in the terminal node, then it becomes redundant to consider gender as an explanatory variable.

Previously, we described hybrid tree-based methods as a two-step algorithm that appears to be very similar to a traditional two-part framework. However, there is a significant difference between them. In the traditional two-part framework, we build the best model based only on information from claim frequency at the beginning, and this claim frequency model will be static in the following process. In other words, there are two optimization processes, one for claim frequency and the other for claim severity. On the other hand, hybrid tree-based methods have only a single optimization process. Based on the hyperparameter settings, the structure of the classification tree, the claim frequency component, can be very flexible. In this sense, hybrid tree-based methods are more similar to {\color{blue}the} Tweedie GLM. This is why it is most suitable to compare our hybrid tree-based methods with {\color{blue}the} Tweedie GLM in the subsequent discussions.
...
\begin{algorithm}[H]
\KwIn{Formula for the tree, formula for the linear model, training dataset $\textbf{x}$, cost-complexity parameter $cp$, maximum depth of final tree $maxdepth$, the threshold for the proportion of zeros in the node to be considered to build a linear model $zeroTreshold$, the choice of the elastic net regularization $glmWhich$, penalty size $glmLambda$.}
...
Else, we build a linear model using observations in this node. If the number of observations in this node is smaller than 40 (a hyperparameter that can be tuned), then we assign the average of the response variable as the prediction; otherwise, we fit ordinary least squares with the elastic net regularization model\;
}
Return Tree structure, linear models at each node, node information, $glmWhich$, $glmLambda$, fitted dataset\;
\caption{Implementation of the hybrid tree-based methods}\label{alg:ht}
\end{algorithm}

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{HTtree.png}
\caption{A simplified illustration of the hybrid tree-based methods} \label{fig:HTtree}
\end{figure}

Figure \ref{fig:HTtree} provides a simplified illustration of the underlying procedure in a hybrid tree-based method. In particular, it assumes two tree depths and observes that during the splitting process, the combination of zeros and non-zeros can vary at different nodes. When all remaining observations in the final node are zeros, the predicted value is zero. When there is a more balanced combination of zeros and non-zeros, the elastic net regularized models are fitted. In the case where there are not enough observations, the reasonable prediction is the average of all the response values of the observations in the final node. In the implementation of the hybrid {\color{blue}tree-based methods}, the maximum tree depth, cost-complexity parameter, the thresholds for the proportion of zeros in the node and minimum observations for building regression, are hyperparameters to be tuned.
...
\]
The first coefficients refer to the intercepts. Because of the equivalence of the compound Poisson-gamma with {\color{blue}the} Tweedie distributions, we generated samples drawn from {\color{blue} a Poisson distribution for the claim frequency and a gamma distribution for the claim severity}.

To illustrate possible drawbacks {\color{blue}of using a} Tweedie GLM, we use different linear coefficients for the frequency part and the severity part. The absolute value of the linear coefficients is the same but with a different sign. In real-life, there is no guarantee that the explanatory variables have equal coefficients in both frequency and severity parts. The response variable generated by the compound Poisson-gamma distribution framework using {\color{blue}the} modified \texttt{rtweedie} function in {\color{blue}the} \texttt{tweedie} R package with frequency and severity as components is presented as follows:
...
Once we have produced the simulated (or synthetic) dataset, we apply both {\color{blue}the} Tweedie GLM and hybrid tree-based methods to predict values of the response variables, given the available set of explanatory variables. The prediction performance between the two models is then compared using several validation measures as summarized in Table \ref{tab:SM1}. In Appendix B, Table \ref{tab:VM} describes details for validation measures. {\color{blue}In many cases, machine learning models often outperform traditional statistical techniques regarding a single validation measure. This is especially true when machine learning models are optimized regarding that validation measure. For this reason, we include more validation measures in the following study in order to draw a stronger conclusion.}
...
From Table \ref{tab:SM1}, we find that the Tweedie GLM's {\color{blue}fitting performance is} worse even under the true model assumption, albeit the presence of noise. As we described in {\color{blue}Section} $\ref{sec:HT}$, after hyperparameter tuning, hybrid tree-based methods with the elastic net regression are able to automatically perform coefficient shrinkage via L2 norm and variable selection based on the tree-structure algorithm and L1 norm. In a simulation setting that mimics real-life, Hybrid tree-based methods provide more promising prediction results than the widely accepted Tweedie GLM.
...
In summary, we used {\color{blue}a} Poisson-gamma distribution {\color{blue}within a} two-part framework, adding some noise that mimics real-life situations to create the simulated datasets. Then, we compared the model accuracy between {\color{blue}the} Tweedie GLM and hybrid tree-based methods. We conclude that even under the linear assumption, hybrid tree-based methods can perform better than {\color{blue}the} Tweedie GLM.

\section{Empirical application} \label{sec:APP}

We compare {\color{blue}the} Tweedie GLM with hybrid tree-based methods using one coverage group from the {\color{blue}Wisconsin Local Government Property Insurance Fund (LGPIF) data. This dataset comes from a project at the University of Wisconsin's actuarial research group, and further details about this dataset can be found on their website.\footnote{https://sites.google.com/a/wisc.edu/local-government-property-insurance-fund/about-the-project?authuser=0}} The coverage group is called building and contents (BC) which provides insurance for buildings and properties {\color{blue}of} local government entities including counties, cities, towns, villages, school districts, and other miscellaneous entities. We draw observations from years 2006 to 2010 as the training dataset and the year 2011 as the test dataset. Table \ref{tab:1} and Table \ref{tab:2} summarize the training dataset and the test dataset, respectively.
...
\subsection{{\color{blue}Conventional techniques}} \label{tweedie-glm}

{\color{blue}To examine and compare model performance, we include three conventional techniques in our analysis.}

We replicate the Tweedie GLM as noted in \citet{frees2016mv}. The linear coefficients for the Tweedie GLM has been provided in Table A11 in \citet{frees2016mv}. It was pointed out that {\color{blue}the} Tweedie GLM may not be ideal for the dataset after visualizing the cumulative density function plot of jittered aggregate losses as depicted in Figure A6 of \citet{frees2016mv}. {\color{blue}In spite of its shortcomings, the Tweedie GLM is used primarily for comparison purposes.}

{\color{blue}In addition, to compare the difference between hybrid and traditional tree-based methods, we apply the CART algorithm, the regression tree, to LGPIF data. We perform a grid search on the training dataset with 10-fold cross-validation. The final model has hyperparameters set with 8 minimum number of observations in a region for the recursive binary split to be attempted, and the cost-complexity pruning parameter (\(cp\)) is 0.05. It is worth mentioning that the regression tree uses the simple average at the terminal nodes as the prediction. }

{\color{blue}
We also employ the deep neural network model (DNN) to compare the prediction performance. Bayesian optimization is employed for hyperparameter tuning and neural network architecture calibration. The final neural network model consists of three hidden layers, which have 284, 156, and 352 nodes and dropout rates of 0.44, 0.49, and 0.11, respectively, each followed by the LeakyReLu activation function.}

\subsection{Calibration results of hybrid tree-based methods} \label{hybrid}

{\color{blue}We employ two hybrid tree-based methods for this empirical application. For the binary classification, we use the CART algorithm, and at the terminal node, we use either GLM with the Gaussian family or the elastic net regression with the Gaussian family. Let us denote these two types of hybrid tree-based methods, respectively, as HTGlm and HTGlmnet. 

For the model calibration, we perform a grid search on the training dataset with 10-fold cross-validation to find the optimal set of hyperparameter values. HTGlm has been built based on the following hyperparameters set:  8 minimum number of observations in a region for the recursive binary split to be attempted,  the cost-complexity pruning parameter (\(cp\)) is \(0.0001\), maximum depth of the tree (\(maxdepth\)) is \(8\), and the threshold for the percentage of the zeros in the node to determine whether to build the linear model or assign \(zero\) as the prediction (\(zeroThreshold\)) is \(0.25\). At first glance, the tuned hyperparameter setting indicates that a fairly deep tree comes with a low \(zeroThreshold\) which is somewhat surprising. However, this is a purely data-driven result of optimizing the RMSE objective loss function. The other hybrid tree-based method, HTGlmnet, has the following hyperparameter setting: minimum observations in a region for splitting is 8, \(cp\) is \(0.0002\), \(maxdepth\) is \(10\), \(zeroThreshold\) is \(0.23\), the balance between L1 norm and L2 norm (\(glmWhich\)) is 1, and size of the regularization parameter by the best cross-validation result (\(glmLambda\)) is \(lambda.min\). 

In regards to the calibration of hybrid tree-based methods, it might be helpful to point out a few observations. Tree-based models' size is controlled by \(maxdepth\) and \(cp\), which constrain the growing tree and prune it when it has grown too large. Apparently, growing a large tree with a small dataset is not ideal. At the terminal node, \(zeroThreshold\) has another function that guards against mistakes caused by the frequency part. To put it another way, if we do not feel comfortable with some positive claims allocated with a certain degree of the proportion of zeros by the tree structure, we can increase the \(zeroThreshold\) and create more linear models at the terminal nodes instead of assigning the zero directly. However, this results in higher computation costs. Furthermore, we find that there could be multiple hyperparameter settings that lead to the optimal solution. Obviously, there are many other options for implementing the optimization process. For example, use the MAE as a validation metric, and employ Bayesian optimization instead of the grid search when we have limited computation resources.}


\subsection{Model performance} \label{model-performance}

We use several validation measures to examine the performance of different models, similar to what was done in the simulation. To make for an easier comparison, we added the heat map in addition to the prediction accuracy table based on various validation measures. We present results separately for the training and the test datasets. {\color{blue}A} rescaled value of 100 (colored blue) indicates it is the best model among the comparisons; on the other hand, {\color{blue}a} rescaled value of 0 (colored red) indicates it is the worst. For further details on such {\color{blue}a} heat map for validation, see \citet{quan2018predictive}.
...
From Figure \ref{fig:trainHeat}, we find that {\color{blue}the} Tweedie GLM has the worst performance of model fit in general, and the CART algorithm and HTGlm have better performance on the model fitting. From Figure \ref{fig:testHeat}, we find that HTGlm has the best performance on the test dataset. It also shows that the hybrid tree-based methods prevent overfitting compared to traditional regression trees using the CART algorithm. {\color{blue}One possible reason is the hybrid structure extrapolates the prediction range by using the regression models at the terminal node. Over all, } hybrid tree-based methods perform consistently well for both the training and test datasets. Because hybrid tree-based methods are considered piecewise linear models, the model generalization is inherited from the linear model. We can also note that HTGlmnet has worse performance when compared to HTGlm, for both the training and test datasets. The advantage of regularization does not demonstrate significant effects on the LGPIF dataset because of the limited number of explanatory variables. {\color{blue}Furthermore, we observe a similar phenomenon as the CART algorithm also performs quite well compared to HTGlmnet, which is also likely a reflection of the fact that there are very few explanatory variables.} However, in the simulation study, we showed the advantage of regularization because the data has a more significant number of explanatory variables. The simulated data may be a better replication of real-life insurance loss compared with the LGPIF data with limited explanatory variables. {\color{blue}It is also worth mentioning the discrepancy in the model performance regarding the Gini Index. Based on the definition of the Gini Index, the calculation involves ranking the predicted value. The Tweedie GLM and DNN predictions do not include a large number of zeros; however, in the hybrid structure, we assign zero prediction to a certain terminal node with a high percentage of zero claims. Consequently, the hybrid structure has an inferior performance regarding the Gini Index, due to the uncertainty of ranking zero predictions. The DNN provides one aspect of the black-box model's prediction performance. LGPIF dataset is not large enough for the DNN to observe the deep double descent phenomenon.} In summary, hybrid tree-based methods outperform {\color{blue}the} Tweedie GLM based on various validation measures.


\section{Visualization and interpretation} \label{sec:visual}

The frequency component of hybrid tree-based methods can clearly be visualized as a tree structure like a simple CART algorithm. Here, we use HTGlm to illustrate the visualization and interpretation of hybrid tree-based methods. In Appendix C, Figure \ref{fig:tree} shows the classification tree model for the frequency. We can follow the tree path and construct the density plot for the response variable for each node. {\color{blue}This process will show straightforward visualization of how the sample changes with every split.} Figure \ref{fig:treePath} shows the classification tree and we highlighted (blue) nodes two depths away from the root. Consequently, we have seven nodes including the root nodes. Here we denote the node with numbers for the purpose of identification. Among highlighted nodes, node four (4), on the left, is the terminal node and the others are intermediate nodes. {\color{blue} Figure \ref{fig:density} shows the density of the response variable for the seven nodes mentioned in Figure \ref{fig:treePath}. For better visualization, Figure \ref{fig:density} presents the logarithm of one plus claim amounts instead of actual claim amounts.} The root node has zero point mass as expected and the black dashed line is the mean of the response variable at the node. After the first split using CoverageBC, the mean shifted towards a different direction and the percentage of the point mass at zero also changed in a different direction. At the fourth terminal node, we can barely see any positive claims with the mean shifting towards zero and this terminal node ultimately assigns zero as the prediction. We can see the classification tree algorithm, done by recursive binary splitting, divides the dataset into subspaces that contain more similar observations. The same phenomenon can also be observed in the CART algorithm, which uses a tree structure to separate similar data to the same group in the terminal node and then uses mean or mode as the predicted value. {\color{blue}For the same reason, the proposed simple hybrid tree-based method applies linear models with the Gaussian family in the terminal node without setting the choice of distribution as a tuning parameter.} After dividing the space of explanatory variables into subspaces, it may be more appropriate to make the distribution assumptions and apply the linear model to the subsamples. In a sense, Hybrid tree-based methods employ a `divide and conquer' algorithm, which may be a promising solution for imbalanced datasets.
...
Figure \ref{fig:treePath} shows the path to terminal nodes and we can see a zero or a one, where a linear model is then fitted, for each terminal node. The tree structure and path can provide an indication of the risk factors based on the split of the explanatory variables. Figure \ref{fig:VI} shows variable importance calculated from the classification tree model for the frequency part. Coverage and deductible information are much more important than location and previous claim history to identify whether the claim happened or not. {\color{blue}It is not surprising that the deductible has an important impact on claim frequency, as a claim occurrence will only occur if the amount exceeds the deductible.} At each terminal node, we can also extract the regression coefficients and interpret the estimated linear regression model. Table \ref{tab:beta} provides the regression coefficients at each of the non-zero terminal nodes. The node number has the same label as in Figure \ref{fig:treePath}. For example, the terminal node 245 can be reached as follows: \(CoverageBC\geq2\), \(TypeCounty\), \(Coverage\geq3.5\), \(lnDeductBC\geq7.4\), \(CoverageBC\geq3.8\), \(lnDeductBC<10\), \(lnDeductBC<9.9\), and we estimated the linear model with regression coefficients: \(Intercept=-172854\), \(CoverageBC=15251\), \(lnDeductBC=16777\), \(NoClaimCreditBC=-16254\). {\color{blue}Similarly, Equation (\ref{eq:1a}) can be specified based on the tree structure and regression coefficients. Hence, we are able to write out the hybrid tree-based method in the piecewise linear form.}

At the terminal node, the linear model is built that solely relies on the prediction accuracy from cross-validation and is based on the regularized algorithm. It does not rely on statistical inference, and indeed, some regression coefficients are not statistically significant. However, we can tune hybrid tree-based methods by balancing prediction accuracy and statistical inference. {\color{blue}For example, when we try to measure the impact of a specific explanatory variable on the claim severity, the coefficient should be statistically significant. To find the reliable coefficient, we might attempt to merge some child nodes or split the dataset further and then build another regression model. It is possible that these attempts may not yield optimal prediction performance due to the fact that statistical inference is prioritized over prediction accuracy.}
...
The following content provides details on the various forms of linear models for {\color{blue}the} claim severity.
...
\end{equation}
where \(\ell(y_i, \beta_{s_{m}0} + \sum_{j=1}^p x_{ij} \beta_{s_{m}j})\) is the negative log-likelihood for sample \(i\) and \(n_m\) is the number of observations in {\color{blue}the} terminal node. For the Gaussian family, denoting the design matrix as $\textbf{X}$, {\color{blue}the estimator for} the coefficient is well known as
\[
\mathbf{\boldsymbol{\widehat{\beta}}}_{s_{m}}  = (\textbf{X}^T\textbf{X})^{-1}\textbf{X}^T \textbf{Y}.
\]

Ridge regression \citep{hoerl1970ridge} achieves better prediction accuracy compared to ordinary least squares because of {\color{blue}the} bias-variance trade-off. In other words, the reduction in the variance term of the coefficient is larger than the increase in its squared bias. It performs coefficient shrinkage and forces its correlated explanatory variables to have similar coefficients. In ridge regression, at each terminal node \(R_{m}\), the linear coefficient \(\mathbf{\boldsymbol{\beta}_{s_{m}}}\) can be determined by
...
\end{equation}
where \(\lambda\) is a tuning hyperparameter that controls shrinkage, and thus the number of selected explanatory variables. For the following discussion, we assume the values of \(x_{ij}\) are standardized so that
\[
